...
/*
	All collection types derive from Collection<CollectableType>

	class Array<T> is a wrapper around c arrays(T name[]),
	it includes useful API functions, notably, range-based for loops.

	index_t is the type for indicies, it is a uint32_t

	find_index_t is the type for when querying for a value, this supports -1 for not found,
	it is uint64_t, unfortunately, this type cannot be printed out to the serial monitor, so
	casting to index_t(uint32_t) is required, because that is supported.
*/
